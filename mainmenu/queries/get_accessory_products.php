<?php
    ...
    // 상품 목록 쿼리
    $sql = "SELECT * FROM products $price_condition category = '액세서리' ORDER BY $order_by LIMIT $offset, $items_per_page";
    $get_accessory_products = mysqli_query($conn, $sql);

    // 전체 상품 개수 쿼리 (페이징용)
    $count_sql = "SELECT COUNT(*) as total FROM products $price_condition category = '액세서리'";
