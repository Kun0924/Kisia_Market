<?php
    ...
    switch ($price_range) {
        ...
        case '200000-up':
            ...
            break;
        default:
            $price_condition = ''; // 전체 보기
    }

    // 상품 목록 쿼리
    $sql = "SELECT * FROM products $price_condition ORDER BY $order_by LIMIT $offset, $items_per_page";
    $get_all_products = mysqli_query($conn, $sql);

    // 전체 상품 개수 쿼리 (페이징용)
    $count_sql = "SELECT COUNT(*) as total FROM products $price_condition";
