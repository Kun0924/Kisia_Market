<?php
    ...
    // URL 파라미터에서 현재 페이지, 정렬 방식, 가격 범위, 검색결과 가져오기
    ...
    // 상품 목록 쿼리
    $sql = "SELECT * FROM products $price_condition name LIKE '%$search_query%' ORDER BY $order_by LIMIT $offset, $items_per_page";
    $get_all_products = mysqli_query($conn, $sql);

    // 전체 상품 개수 쿼리 (페이징용)
    $count_sql = "SELECT COUNT(*) as total FROM products $price_condition name LIKE '%$search_query%'";
