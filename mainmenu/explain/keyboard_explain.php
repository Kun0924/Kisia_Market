                ...
                if (mysqli_num_rows($get_keyboard_products) > 0) {
                    while ($row = mysqli_fetch_assoc($get_keyboard_products)) {
                        if ($row['id'] == $product_id) {
                            ...
                            echo '<div class="tabs">';
                            echo '<button class="tab-btn active" onclick="openTab(\'description\')">상품 설명</button>';
                            echo '<button class="tab-btn" onclick="openTab(\'reviews\')">리뷰</button>';
                            echo '</div>';
